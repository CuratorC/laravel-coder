...
class CreateUsersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->decimal('all_performance_fee', 20, 8)->default(0)->comment('监审费用');
            $table->decimal('give_out_performance_fee', 20, 8)->default(0)->comment('监审费用');
            $table->decimal('wait_supervision_fee', 20, 8)->default(0)->comment('监审费用');
            $table->decimal('freeze_supervision_fee', 20, 8)->default(0)->comment('监审费用');
